<template>
  <section class="section">
        <b-field>
            <b-input placeholder="City, Country"
                type="search"
                icon="magnify"
                v-model="newCity"
            >
    ...
    <div class="columns is-desktop" v-if="weatherData.name">
      ...
      <card
        :title="weatherData.weather.summary.title"
        icon="brightness-5"
      >
      ...
      >
        {{ (weatherData.weather.temperature.actual - 273.15).toFixed(1)  }} ºC ({{ (weatherData.weather.temperature.feelsLike - 273.15).toFixed(1) }} ºC)
      </card>
